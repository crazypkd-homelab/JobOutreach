...
describe("accounts API", () => {
  ...
  it("makes the first account default and moves the flag when another claims it", async () => {
    ...
    const list: AccountView[] = await (await app.request("/api/accounts", { headers: { Cookie: cookie } })).json();
  ...
  it("promotes another account when the default is deleted", async () => {
    ...
    expect((await app.request(`/api/accounts/${first.id}`, { method: "DELETE", headers: { Cookie: cookie } })).status).toBe(204);
  ...
  it("rejects invalid input and unknown ids", async () => {
    ...
    expect((await app.request("/api/accounts/999", { headers: { Cookie: cookie } })).status).toBe(404);
  });
  ...
  it("lists models for a key", async () => {
    ...
    const res = await app.request(`/api/accounts/${account.id}/models`, { headers: { Cookie: cookie } });
  ...
  it("records success on a passing test", async () => {
    ...
    const res = await app.request(`/api/accounts/${account.id}/test`, { method: "POST", headers: { Cookie: cookie } });
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual({ ok: true, model: "gpt-oss:20b" });

    const after: AccountView = await (await app.request(`/api/accounts/${account.id}`, { headers: { Cookie: cookie } })).json();
  ...
  it("clears the quota flag once a test passes again", async () => {
    ...
    await app.request(`/api/accounts/${account.id}/test`, { method: "POST", headers: { Cookie: cookie } });
...
describe("prompts API", () => {
  it("lists both prompts with their paths", async () => {
    const res = await app.request("/api/prompts", { headers: { Cookie: cookie } });
  ...
  it("404s on an unknown prompt", async () => {
    expect((await app.request("/api/prompts/nope", { headers: { Cookie: cookie } })).status).toBe(404);
  });

  it("resets a prompt", async () => {
    const res = await app.request("/api/prompts/extract_jd/reset", { method: "POST", headers: { Cookie: cookie } });
    expect(res.status).toBe(200);
