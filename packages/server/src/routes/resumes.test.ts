...
describe("resumes API", () => {
  ...
  it("uploads a PDF resume and extracts text", async () => {
    ...
    const res = await app.request("/api/resumes", { method: "POST", body: form });
  ...
  it("rejects missing file field", async () => {
    const res = await app.request("/api/resumes", { method: "POST", body: new FormData() });
    expect(res.status).toBe(400);
  });
  ...
  it("gets a resume by id", async () => {
    ...
    const created: ResumeView = await (await app.request("/api/resumes", { method: "POST", body: form })).json();
  ...
  it("returns 404 for unknown resume", async () => {
    expect((await app.request("/api/resumes/999")).status).toBe(404);
  });
  ...
  it("deletes a resume", async () => {
    ...
    expect((await app.request(`/api/resumes/${created.id}`, { method: "DELETE" })).status).toBe(204);
    expect((await app.request(`/api/resumes/${created.id}`)).status).toBe(404);
  });
});
